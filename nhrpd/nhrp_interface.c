...
int nhrp_interface_up(int cmd, struct zclient *client, zebra_size_t length, vrf_id_t vrf_id)
{
	...
	ifp = zebra_interface_state_read(client->ibuf, vrf_id); /* it updates iflist */
	if (ifp == NULL)
		return 0;
...
int nhrp_interface_address_add(int cmd, struct zclient *client, zebra_size_t length, vrf_id_t vrf_id)
{
	...
	ifc = zebra_interface_address_read(ZEBRA_INTERFACE_ADDRESS_ADD, client->ibuf, vrf_id);
	if (ifc == NULL)
		return 0;
...
int nhrp_interface_address_delete(int cmd, struct zclient *client, zebra_size_t length, vrf_id_t vrf_id)
{
	...
	ifc = zebra_interface_address_read(ZEBRA_INTERFACE_ADDRESS_DELETE, client->ibuf, vrf_id);
	if (ifc == NULL)
		return 0;
